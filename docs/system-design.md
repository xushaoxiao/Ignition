...
> 状态：第 13 节的三个前置问题仍待确认；收入链路的骨架已落地
>
> **这份文档描述的是目标状态，不是当前状态。** 已实现到什么程度、哪些是占位
> 实现，见仓库根目录的 `README.md`。两者冲突时以 README 为准，并回来修正本文。
> 归因规则的对外口径见 `docs/attribution-policy-v1.md`。
